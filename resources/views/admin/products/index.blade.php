...
<div class="card shadow-sm mb-4">
<div class="card-body">
<form action="{{ route('admin.genset.updateBrand',$selectedBrand->id) }}"
...
<option value="1" {{ $selectedBrand->is_active ? 'selected' : '' }}>Active</option>
<option value="0" {{ !$selectedBrand->is_active ? 'selected' : '' }}>Inactive</option>
</select>
</div>
...
action="{{ route('admin.genset.deleteSpec',$spec->id) }}"
method="POST">
@csrf
@method('DELETE')
</form>
...
<form action="{{ route('admin.genset.storeBrand') }}"
method="POST"
